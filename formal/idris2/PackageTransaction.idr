...
validPackage : Package -> Bool
validPackage package =
  packageName package /= "" &&
  versionIndex package /= 0 &&
  provenanceRoot package /= 0 &&
  artifactRoot package /= 0 &&
  abiVersion package /= 0 &&
  serviceClass package /= 0
...
upgradeExact name expected replacement (package :: rest) =
  if packageName package == name
    then if replacement == 0
      then Left InvalidPackageIdentity
      else if versionIndex package == expected
      then Right
        (MkPackage
          name
          replacement
          (provenanceRoot package)
...
applyMutation packages (Install package) =
  if validPackage package
    then case findPackage (packageName package) packages of
      Just installed => Left AlreadyInstalled
      Nothing => Right (packages ++ [package])
    else Left InvalidPackageIdentity
...
sampleStage =
  stage
    (MkLedger [])
    [ Install (MkPackage "boulder" 1 0x424f554c445052 0x424f554c444552 1 1)
    , Install (MkPackage "crest" 3 0x43524553545052 0x4352455354 1 2)
    ]
