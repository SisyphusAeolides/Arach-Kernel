...
|---|---|---|
| Kernel core | Memory, interrupt, process, capability, driver, filesystem, networking, and native/Linux execution-ABI metadata build and pass host tests; a measured Linux-personality ring-3 probe exercises identity, `write`, anonymous `mmap`/`munmap`, `brk`, and `exit_group` through real generation-bound lifecycle/page-table paths. Bounded process-owned `eventfd`, `timerfd`, `poll`, and `epoll` wake paths are implemented; other Linux calls fail closed | Keep the Granite/QEMU Linux-probe gate green, then admit file, signal, IPC, device, and graphics syscalls one measured surface at a time |
| System bootstrap | The pinned Granite/Arach/Push C0 bundle executes under QEMU/OVMF in CI and emits measured ring-3 syscall evidence; revision `b396d3a7fc6538eacc60058d7067bebe9de43537` is the current qualified release | Keep the exact-revision gate green, then promote this C0 path toward a native COSMIC login/session qualification |
