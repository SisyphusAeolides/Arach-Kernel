...
- one fail-closed, eight-object `DT_NEEDED` engine whose freestanding C linker
  ...
  measured four-object diamond applies five real `R_X86_64_RELATIVE` writes,
  installs one real `R_X86_64_TPOFF64` into a bounded Variant-II static TLS
  arena, binds four PLT edges, seals every object R/RW/RX, executes four
  dependency-first initializers, and calls through both branches and their
  shared provider in QEMU.
...
2. Extend the measured bounded graph linker with symbol versions, finalization,
   and general dynamic TLS beyond its current initial static TLS arena.
