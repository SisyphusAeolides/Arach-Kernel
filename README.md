...
|---|---|---|
| Kernel core | Memory, interrupt, process, capability, driver, filesystem, networking, and native/Linux execution-ABI metadata build and pass host tests; Linux syscalls fail closed instead of entering the native table | Boot the native kernel through Granite, execute the ring-3 probe, then admit Linux syscall implementations one measured surface at a time |
