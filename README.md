...
|---|---|---|
| Kernel core | Memory, interrupt, process, capability, driver, filesystem, networking, and native/Linux execution-ABI metadata build and pass host tests; a measured Linux-personality ring-3 probe exercises identity, `write`, anonymous `mmap`/`munmap`, `brk`, and `exit_group` through real generation-bound lifecycle/page-table paths while other Linux calls fail closed | Keep the Granite/QEMU Linux-probe gate green, then admit file, signal, IPC, device, and graphics syscalls one measured surface at a time |
