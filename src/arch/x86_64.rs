...
impl Architecture for X86_64 {
    ...
    fn hardware_thread_id() -> u32 {
        // CPUID is available in 64-bit mode and leaf zero is always defined.
        // Later leaves are queried only after checking the maximum.
        let maximum_leaf = core::arch::x86_64::__cpuid(0).eax;
        for leaf in [0x1f, 0x0b] {
            if maximum_leaf >= leaf {
                // `leaf` is supported according to the value reported by leaf zero.
        ...
        // CPUID leaf one is mandatory on x86_64.
    ...
    fn save_and_disable_interrupts() -> InterruptState {
        let flags: usize;
        // SAFETY: Arach executes this at ring 0. Reading RFLAGS and clearing
...
pub unsafe fn enable_execute_disable() -> Result<(), ExecuteDisableError> {
    // The extended maximum-leaf query is defined on x86_64.
    let maximum_extended_leaf = core::arch::x86_64::__cpuid(0x8000_0000).eax;
    if maximum_extended_leaf < 0x8000_0001
        // The preceding maximum-leaf check admits this query.
