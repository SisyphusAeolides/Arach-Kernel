...
/// # Safety
///
/// The GDT above must be active, `arach_syscall_entry` and its dedicated
...
unsafe fn configure_syscall_entry(registration: CpuRegistration) -> Result<(), PrivilegeError> {
    ...
    unsafe extern "C" {
        fn arach_syscall_entry();
    }
    let entry_address = arach_syscall_entry as *const () as usize as u64;
...
pub unsafe fn run_user_probe(
    ...
    unsafe { arach_enter_ring3_probe(entry_point, stack_pointer, page_table_root) };
...
pub unsafe fn enter_user_process(
    ...
    unsafe { arach_enter_user_process(entry_point, stack_pointer, page_table_root) }
}
