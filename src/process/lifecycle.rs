...
/// Returns a snapshot only when the full PID generation still names the
/// installed process. This prevents a recycled PID from inheriting a former
/// service's authority.
pub fn snapshot_exact(handle: ProcessHandle) -> Option<ProcessSnapshot> {
    TABLE
        .lock()
