...
use crate::lease_lattice::LeaseToken;
...
pub fn initialize(
    ...
    command_lease: LeaseToken,
) -> Result<(), PlaneDriverInitError> {
    if command_lease == LeaseToken::INVALID {
        return Err(PlaneDriverInitError::InvalidLease);
    }

    INGRESS_PAGE.initialize(1);
    INGRESS_PAGE.publish_capability(command_lease.raw());
