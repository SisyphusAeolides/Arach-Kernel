...
use crate::capability::{Authority, Capability, DeviceMemoryControl};
...
// This is deliberately an object identity, never a mapped address.  It is
// published once by Arach after the firmware driver has retained and
// verified the display.  User processes can only reach it through the
// authenticated Crest presentation syscall below.
static CREST_PRESENTER_OBJECT: AtomicU64 = AtomicU64::new(0);
...
/// Makes an already retained firmware display available to the Crest-only
/// present path.  Rebinding to another object is rejected rather than silently
/// retargeting a live desktop session.
pub fn bind_crest_presenter(object: u64) -> Result<(), FirmwareDisplayError> {
    let snapshot = snapshot(object).ok_or(FirmwareDisplayError::NotFound)?;
    if snapshot.retain_count == 0 {
        return Err(FirmwareDisplayError::StaleObject);
    }
    match CREST_PRESENTER_OBJECT.compare_exchange(0, object, Ordering::AcqRel, Ordering::Acquire) {
        Ok(_) => Ok(()),
        Err(existing) if existing == object => Ok(()),
        Err(_) => Err(FirmwareDisplayError::DomainMismatch),
    }
}

// Crest's 640×400 BGRA workspace is 1,024,000 bytes.  This bound is shared
// with the syscall staging ceiling and remains below the one-MiB mapping cap.
const MAXIMUM_PRESENT_SOURCE_BYTES: usize = 1024 * 1024;
const MAXIMUM_PRESENT_MAPPING_BYTES: u64 = 1024 * 1024;

/// Copies Crest's private BGRA8888 frame into the retained firmware scanout.
///
/// The presenter identity is an internal kernel object and this function has
/// no user-address or raw-framebuffer parameters.  It maps one bounded stripe
/// at a time and drops every mapping before progressing, so a desktop frame
/// cannot monopolize the MMIO aperture.
pub fn present_crest_bgra8888(
    source: &[u8],
    source_width: u32,
    source_height: u32,
    source_pitch: u32,
) -> Result<u64, FirmwareDisplayError> {
    let object = CREST_PRESENTER_OBJECT.load(Ordering::Acquire);
    if object == 0 {
        return Err(FirmwareDisplayError::NotFound);
    }
    let snapshot = snapshot(object).ok_or(FirmwareDisplayError::NotFound)?;
    if snapshot.retain_count == 0 || source_width == 0 || source_height == 0 {
        return Err(FirmwareDisplayError::StaleObject);
    }
    let required_pitch = source_width
        .checked_mul(4)
        .ok_or(FirmwareDisplayError::AddressOverflow)?;
    let required_bytes = usize::try_from(
        u64::from(source_pitch)
            .checked_mul(u64::from(source_height))
            .ok_or(FirmwareDisplayError::AddressOverflow)?,
    )
    .map_err(|_| FirmwareDisplayError::AddressOverflow)?;
    if source_pitch < required_pitch
        || required_bytes > MAXIMUM_PRESENT_SOURCE_BYTES
        || source.len() != required_bytes
    {
        return Err(FirmwareDisplayError::InvalidEvidence);
    }
    if !matches!(
        snapshot.evidence.format,
        FRAMEBUFFER_FORMAT_XRGB8888 | FRAMEBUFFER_FORMAT_XBGR8888 | FRAMEBUFFER_FORMAT_RGB565
    ) {
        return Err(FirmwareDisplayError::UnsupportedFormat);
    }

    let rows = (MAXIMUM_PRESENT_MAPPING_BYTES / u64::from(snapshot.evidence.pitch))
        .max(1)
        .min(u64::from(snapshot.evidence.height)) as u32;
    // SAFETY: This is Arach-owned display dispatch after exact Crest caller
    // authentication.  The scoped token is never exposed or retained beyond
    // a single synchronous MMIO stripe operation.
    let authority = unsafe { Authority::assume_root() };
    let device_memory = authority.grant::<DeviceMemoryControl>();
    let mut destination_y = 0;
    let mut root = mix(object, u64::from(snapshot.generation));
    while destination_y < snapshot.evidence.height {
        let stripe_height = rows.min(snapshot.evidence.height - destination_y);
        let offset = u64::from(destination_y)
            .checked_mul(u64::from(snapshot.evidence.pitch))
            .ok_or(FirmwareDisplayError::AddressOverflow)?;
        let length = usize::try_from(
            u64::from(stripe_height)
                .checked_mul(u64::from(snapshot.evidence.pitch))
                .ok_or(FirmwareDisplayError::AddressOverflow)?,
        )
        .map_err(|_| FirmwareDisplayError::AddressOverflow)?;
        let mapping = FirmwareDisplayMapping::map_span(object, offset, length, &device_memory)?;
        let mut result = Ok(());
        'pixels: for y in destination_y..destination_y + stripe_height {
            let source_y = (u64::from(y) * u64::from(source_height)
                / u64::from(snapshot.evidence.height)) as u32;
            for x in 0..snapshot.evidence.width {
                let source_x = (u64::from(x) * u64::from(source_width)
                    / u64::from(snapshot.evidence.width)) as u32;
                let source_offset = usize::try_from(
                    u64::from(source_y)
                        .checked_mul(u64::from(source_pitch))
                        .and_then(|row| row.checked_add(u64::from(source_x) * 4))
                        .ok_or(FirmwareDisplayError::AddressOverflow)?,
                )
                .map_err(|_| FirmwareDisplayError::AddressOverflow)?;
                let pixel = match source.get(source_offset..source_offset + 4) {
                    Some(pixel) => pixel,
                    None => {
                        result = Err(FirmwareDisplayError::InvalidEvidence);
                        break 'pixels;
                    }
                };
                ...
                if let Err(error) = mapping.write_pixel(x, y, color) {
                    result = Err(error);
                    break 'pixels;
                }
                root = mix(
                    root,
                    u64::from(pixel[0]) | (u64::from(pixel[1]) << 8) | (u64::from(pixel[2]) << 16),
                );
            }
        }
        let close_status = mapping.close(&device_memory);
        result?;
        if close_status != STATUS_OK {
            return Err(FirmwareDisplayError::Unmap(close_status));
        }
        destination_y += stripe_height;
    }
    Ok(root)
}
