...
/// Determines when the controller-reset authority is exercised.
///
/// A controller behind a live DMA-remapping unit can require its first reset
/// only after the exact DMA domain is installed.  The deferred policy still
/// requires the initial controller to be halted and ready; its caller must
/// complete the reset before releasing that domain.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ResetPolicy {
    BeforeReady,
    DeferUntilMappedRuntime,
}
...
impl<'a> TakeoverMachine<'a> {
    ...
    pub fn bind_measured_aperture(
        ...
        self.phase = match self.config.reset_policy {
            ResetPolicy::BeforeReady => TakeoverPhase::RequestReset,
            ResetPolicy::DeferUntilMappedRuntime => TakeoverPhase::VerifyReadyHalted,
        };
        Ok(())
    }
...
mod tests {
    ...
    fn config<'a>(protocols: &'a [Usb2ProtocolPorts]) -> TakeoverConfig<'a> {
        TakeoverConfig {
            ...
            maximum_ports: 8,
            reset_policy: ResetPolicy::BeforeReady,
    ...
    #[test]
    fn deferred_reset_policy_returns_a_halted_controller_without_issuing_reset() {
        let mut configured = config(&[]);
        configured.reset_policy = ResetPolicy::DeferUntilMappedRuntime;
        let mut machine = TakeoverMachine::new(configured).unwrap();
        let mut registers = FakeRegisters::qemu();
        let mut deadline = FakeDeadline::new(8);
        drive_to_phase(
        ...
        machine.bind_measured_aperture(4096).unwrap();
        assert_eq!(machine.phase(), TakeoverPhase::VerifyReadyHalted);
        assert_eq!(registers.reset_requests, 0);

        let TakeoverObservation::Ready(ready) =
            machine.step(&mut registers, &mut deadline).unwrap()
        else {
            panic!("deferred policy did not produce a halted controller");
        };
        assert_eq!(
            ready.command() & (COMMAND_RUN_STOP | COMMAND_HOST_CONTROLLER_RESET),
            0
