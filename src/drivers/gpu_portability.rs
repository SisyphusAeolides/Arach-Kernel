...
use super::drivernet::fingerprint::{
    ...
    NVIDIA_ARCHITECTURE_TURING_OR_NEWER, TOPOLOGY_BOOT_DISPLAY, TOPOLOGY_CONFIG_INCOMPLETE,
    TOPOLOGY_FIRMWARE_FRAMEBUFFER, TOPOLOGY_HOTPLUG_PORT, TOPOLOGY_INVENTORY_OVERFLOW,
    TOPOLOGY_IOMMU_ISOLATED, TOPOLOGY_IOMMU_PRESENT, TOPOLOGY_VIRTUAL_MACHINE, VENDOR_AMD,
    VENDOR_INTEL, VENDOR_NVIDIA, VENDOR_VIRTIO, VENDOR_VMWARE, nvidia_architecture_hint,
};
...
fn native_manifest(driver_id: u64, vendor_id: u16, priority: u16) -> GpuCompatibilityManifest {
    ...
    let mut manifest = GpuCompatibilityManifest {
    ...
    if vendor_id == VENDOR_NVIDIA {
        manifest.architecture_mask = NVIDIA_ARCHITECTURE_TURING_OR_NEWER;
        manifest.architecture_value = NVIDIA_ARCHITECTURE_TURING_OR_NEWER;
    }
    manifest
}
...
pub fn portable_evidence(fingerprint: &GpuFingerprint) -> GpuDeviceEvidence {
    ...
    GpuDeviceEvidence {
        ...
        architecture_hint: if fingerprint.vendor_id == VENDOR_NVIDIA {
            nvidia_architecture_hint(fingerprint.device_id)
        } else {
            0
        },
        bootrom_revision: 0,
...
mod tests {
    ...
    #[test]
    fn hermes_requires_explicit_turing_or_later_architecture_evidence() {
        let mut fingerprint = GpuFingerprint::EMPTY;
        fingerprint.vendor_id = VENDOR_NVIDIA;
        ...
        fingerprint.evidence_root = 0x21;
        let resolver = GpuPortabilityResolver::new(0x21).unwrap();
        assert!(matches!(
            resolver.prove(DriverStrategy::HermesNvidia, &fingerprint),
            Err(PortabilityFault::Rejected(_))
        ));
