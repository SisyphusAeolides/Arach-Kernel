use super::fingerprint::{
    ...
    VENDOR_VIRTIO, VENDOR_VMWARE, is_nvidia_turing_or_newer,
};
...
pub const REASON_UNSUPPORTED_ARCHITECTURE: u32 = 1 << 9;
...
fn gate_strategy(
    ...
    let vendor_gate = match strategy {
        DriverStrategy::HermesNvidia => {
            fingerprint.vendor_id == VENDOR_NVIDIA
                && is_nvidia_turing_or_newer(fingerprint.device_id)
    ...
    if vendor_gate {
        ...
        let reason = if strategy == DriverStrategy::HermesNvidia
            && fingerprint.vendor_id == VENDOR_NVIDIA
            && !is_nvidia_turing_or_newer(fingerprint.device_id)
        {
            REASON_UNSUPPORTED_ARCHITECTURE
        } else {
            REASON_UNKNOWN_VENDOR
        };
        (false, reason)
    }
}
