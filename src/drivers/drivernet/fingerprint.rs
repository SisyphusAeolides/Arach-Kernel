...
/// Portable-ABI architecture bits for NVIDIA graphics processors.
///
/// The low compatibility bit is present on every explicitly recognized
/// Turing-or-newer device. Family bits preserve the measured classification
/// for a codec without making an unknown future PCI ID look supported.
...
/// Classifies only PCI device-ID bands whose NVIDIA architecture is known to
/// this kernel. It is a coarse admission gate; codec transport negotiation
/// still proves the specific GSP personality before any command is issued.
pub const fn nvidia_architecture_hint(device_id: u16) -> u32 {
    let family = match device_id {
        0x1e00..=0x1fff | 0x2180..=0x21ff => NVIDIA_ARCHITECTURE_TURING,
        0x2000..=0x20ff | 0x2200..=0x22ff | 0x2400..=0x25ff => NVIDIA_ARCHITECTURE_AMPERE,
        0x2300..=0x23ff => NVIDIA_ARCHITECTURE_HOPPER,
        0x2600..=0x28ff => NVIDIA_ARCHITECTURE_ADA,
        0x2900..=0x2fff => NVIDIA_ARCHITECTURE_BLACKWELL,
        _ => return 0,
    };
    NVIDIA_ARCHITECTURE_TURING_OR_NEWER | family
}

pub const fn is_nvidia_turing_or_newer(device_id: u16) -> bool {
    nvidia_architecture_hint(device_id) & NVIDIA_ARCHITECTURE_TURING_OR_NEWER != 0
}
...
impl PciConfigReader for LegacyConfigurationReader {
    ...
    fn read_u32(&self, address: PciFunctionAddress, offset: u16) -> Result<u32, FingerprintError> {
        ...
        // the globally serialized PCI mechanism owned by Arach.
...
mod tests {
    ...
    #[test]
    fn nvidia_generation_classifier_accepts_turing_and_later_but_not_volta() {
        assert!(!is_nvidia_turing_or_newer(0x1db6)); // GV100 / Volta
        assert!(is_nvidia_turing_or_newer(0x1e04)); // TU102
        assert!(is_nvidia_turing_or_newer(0x2182)); // TU116
        assert!(is_nvidia_turing_or_newer(0x2204)); // GA102
        assert!(is_nvidia_turing_or_newer(0x2684)); // AD102
        assert!(is_nvidia_turing_or_newer(0x2b85)); // GB202
