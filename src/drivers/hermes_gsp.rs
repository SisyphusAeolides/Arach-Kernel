...
use super::{
    drivernet::fingerprint::{GpuFingerprint, is_nvidia_turing_or_newer},
...
struct Runtime<const PENDING: usize> {
    ...
    /// OS-wide phase lattice shared with Crest and the Idris HermesAuthority
    /// model. Online is never published without every measured gate.
...
impl<const PENDING: usize> Runtime<PENDING> {
    fn new(
        ...
        let mut manifold = HermesManifold::dark(epoch);
        // Bind starts from a measured PCI identity; probe is the first legal
        // step on the formal lattice and cannot be skipped later.
        let _ = manifold.observe_probe(true);
        Ok(Self {
            epoch,
            next_sequence: 1,
            command_producer: 0,
    ...
    pub fn stage(
        ...
        match firmware {
            Some(image) => {
                ...
                // Firmware measurement is a hard lattice step. Staging without
                // a sealed image leaves Hermes in Probed and cannot ignite.
                let _ = self.runtime.manifold.observe_firmware(true);
            ...
            None => {
                ...
                // Codec-only personalities measure the empty firmware gate as
                // satisfied so the phase lattice can still reach Queued/Online.
                let _ = self.runtime.manifold.observe_firmware(true);
            }
        }
        ...
        // Rings + IOMMU isolation form the Queued phase. Domain id is the
        // runtime epoch (never zero after bind); the formal model rejects 0.
        let domain_id = self.runtime.epoch.max(1);
        let _ = self.runtime.manifold.arm_queues(true, domain_id);
    ...
    Hermes<'a, Backend, Staged, PENDING>
{
    pub fn ignite(
        ...
        // Feature negotiation is its own lattice step. Ill-formed feature sets
        // fail closed here so Online never publishes an Agda-illegal lattice.
        self.runtime
            .manifold
            .negotiate(negotiated_features)
            .map_err(|_| HermesFault::RequiredFeatureMissing)?;
        // WPR/mailbox/ready were observed by the ignite/boot stage sequence
        // above; seal Online only when the manifold accepts every gate.
        self.runtime
            .manifold
            .ignite(true, true, true)
            .map_err(|_| HermesFault::ProtocolMismatch)?;
...
impl<Backend: HermesPlatform + ?Sized, const PENDING: usize> Hermes<'_, Backend, Online, PENDING> {
    ...
    /// Formal phase lattice published to Crest through Quantum Crest snapshots.
    pub const fn manifold_phase(&self) -> HermesPhase {
        self.runtime.manifold.phase()
    }

    pub const fn manifold(&self) -> HermesManifold {
        self.runtime.manifold
    }
...
mod tests {
    ...
    fn portable_evidence() -> GpuDeviceEvidence {
        ...
        evidence.architecture_hint = nvidia_architecture_hint(identity.device_id);
    ...
    #[test]
    fn discovery_rejects_pre_turing_nvidia_before_personality_negotiation() {
        let mut fingerprint = GpuFingerprint::EMPTY;
        fingerprint.vendor_id = NVIDIA_VENDOR_ID;
        fingerprint.device_id = 0x1db6; // GV100 / Volta
        fingerprint.class_code = PCI_CLASS_DISPLAY;
        assert_eq!(
            HermesDiscovery::from_fingerprint(&fingerprint),
