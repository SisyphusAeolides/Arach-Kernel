...
const USBSTS_HOST_CONTROLLER_FATAL_ERROR: u32 = 1 << 12;
...
const RT_IMAN: u32 = 0x00;
const RT_ERSTSZ: u32 = 0x08;
const RT_ERSTBA: u32 = 0x10;
const RT_ERDP: u32 = 0x18;
...
pub enum XhciRuntimeInvariant {
    ...
    ControllerStartReadbackMismatch,
...
impl XhciPreparedRuntime {
    ...
    {
        self.validate_running_receipt(running)?;
        let status_offset = checked_offset(self.operational_offset, 4)?;
        let status = registers
            .read32(status_offset)
            .map_err(XhciRuntimeError::Register)?;
        if status & (USBSTS_HOST_CONTROLLER_ERROR | USBSTS_HOST_CONTROLLER_FATAL_ERROR) != 0 {
        ...
        if let Some(evidence) = completion {
            ...
            write_u64_register(registers, self.event_dequeue_pointer_offset(), dequeue)?;
...
/// Installs exact requester mappings before any halted-controller register is
/// allowed to publish an arena address.  Some controllers fetch ERST as soon
/// as its high dword is written, even while HCHalted and bus mastering remain
/// clear, so mapping must precede [`prepare_halted_from_evidence`].
pub fn bind_halted_dma<S>(
...
where
    S: XhciRingStorage,
{
    bind_core_regions(domain, storage, evidence.generation, secret)
}
...
/// Clears every retained DMA-bearing register while the controller is proven
/// halted, before a new translated runtime is programmed.
///
/// This is the required containment bridge for a reset deferred until the
/// IOMMU domain exists: no stale firmware event-ring or command-ring address
/// may survive into the new requester context.
pub fn scrub_halted_from_evidence<R>(
    evidence: XhciRuntimeEvidence,
    aperture_length: u64,
    registers: &mut R,
    ...
    R: XhciRuntimeRegisters,
{
    ...
    let runtime_end = runtime_offset
    ...
    scrub_halted_registers(registers, operational_offset, runtime_offset)
}

/// Captures the page-aligned event-dequeue address retained by a halted
/// controller before its runtime registers are scrubbed.
///
/// A nonzero result is not DMA authority.  It is a narrow containment input
/// for the one reset that clears firmware-owned event state after an IOMMU
/// context becomes live.
pub fn halted_event_dequeue_page<R>(
    evidence: XhciRuntimeEvidence,
    aperture_length: u64,
    registers: &mut R,
) -> Result<Option<u64>, XhciRuntimeError<(), R::Error>>
where
    R: XhciRuntimeRegisters,
{
    let runtime_offset = evidence.snapshot.runtime_offset;
    let erdp = runtime_offset
        .checked_add(RUNTIME_INTERRUPTER)
        .and_then(|offset| offset.checked_add(RT_ERDP))
        .ok_or(XhciRuntimeError::Invariant(
            XhciRuntimeInvariant::RegisterOffsetOverflow,
        ))?;
    let end =
        erdp.checked_add(core::mem::size_of::<u64>() as u32)
            .ok_or(XhciRuntimeError::Invariant(
                XhciRuntimeInvariant::RegisterOffsetOverflow,
            ))?;
    if u64::from(end) > aperture_length {
        return Err(XhciRuntimeError::Invariant(
            XhciRuntimeInvariant::RegisterOffsetOverflow,
        ));
    }
    verify_stopped(registers, u32::from(evidence.snapshot.capability_length))?;
    let low = registers.read32(erdp).map_err(XhciRuntimeError::Register)?;
    let high = registers
        .read32(erdp + 4)
        .map_err(XhciRuntimeError::Register)?;
    ...
    if !evidence.snapshot.supports_64_bit_addresses && high != 0 {
        return Err(XhciRuntimeError::Invariant(
            XhciRuntimeInvariant::AddressExceeds32Bit(address),
        ));
    }
    Ok(Some(address & !(abyss::paging::PAGE_SIZE as u64 - 1)))
}

/// Resets a halted controller after a temporary IOMMU containment mapping has
/// been installed for any firmware-owned event dequeuer it retained.
pub fn reset_halted_from_evidence<R>(
    evidence: XhciRuntimeEvidence,
    registers: &mut R,
    poll_limit: u32,
...
where
    R: XhciRuntimeRegisters,
{
    let root = mix(evidence.reset_ready_root, u64::from(evidence.generation));
    reset_halted_controller(
        registers,
        u32::from(evidence.snapshot.capability_length),
        root,
        XhciRunStopReceipt {
            start_polls: 0,
            halt_polls: 0,
    ...
    R: XhciRuntimeRegisters,
{
    ...
    let iman = runtime_base
        .checked_add(RUNTIME_INTERRUPTER)
        .and_then(|offset| offset.checked_add(RT_IMAN))
        .ok_or(XhciRuntimeError::Invariant(
            XhciRuntimeInvariant::RegisterOffsetOverflow,
        ))?;
    let erstba = erstsz
    ...
    registers
        .write32(iman, IMAN_INTERRUPT_ENABLE)
        .map_err(XhciRuntimeError::Register)?;
    ...
    R: XhciRuntimeRegisters,
{
    ...
    write_u64_register(registers, erdp, 0)?;
    registers
        .write32(iman, 0)
        .map_err(XhciRuntimeError::Register)?;
    write_u64_register(registers, erstba, 0)?;
    registers
        .write32(erstsz, 0)
        .map_err(XhciRuntimeError::Register)?;
    write_u64_register(registers, crcr, 0)?;
    write_u64_register(registers, dcbaap, 0)?;
    registers
        .write32(config, 0)
        .map_err(XhciRuntimeError::Register)?;
    verify_stopped(registers, operational_offset)
    ...
    R: XhciRuntimeRegisters,
{
    ...
    let command = registers
        ...
        .map_err(XhciRuntimeError::Register)?;
    let status_offset = checked_offset(operational_offset, 4)?;
    let running_command = command | USBCMD_RUN_STOP | USBCMD_INTERRUPT_ENABLE;
    registers
        .write32(operational_offset, running_command)
        .map_err(XhciRuntimeError::Register)?;

    let readback = registers
        .read32(operational_offset)
        .map_err(XhciRuntimeError::Register)?;
    if readback & (USBCMD_RUN_STOP | USBCMD_INTERRUPT_ENABLE)
        != USBCMD_RUN_STOP | USBCMD_INTERRUPT_ENABLE
    {
        let _ = registers.write32(operational_offset, command);
        return Err(XhciRuntimeError::Invariant(
            XhciRuntimeInvariant::ControllerStartReadbackMismatch,
        ));
    }
...
fn write_address<S, R: XhciRuntimeRegisters>(
    ...
    if supports_64_bit_addresses {
        write_u64_register(registers, offset, address)
    } else {
...
/// xHCI's 64-bit register pairs are independently addressed 32-bit MMIO
/// registers.  Write the high dword last so hardware observes a complete
/// pointer only after its low dword is in place.
fn write_u64_register<S, R: XhciRuntimeRegisters>(
    registers: &mut R,
    offset: u32,
    value: u64,
) -> Result<(), XhciRuntimeError<S, R::Error>> {
    let high = checked_offset(offset, 4)?;
    registers
        .write32(offset, value as u32)
        .map_err(XhciRuntimeError::Register)?;
    registers
...
mod tests {
    ...
    #[test]
    fn address_programming_writes_ac64_pairs_and_preserves_32_bit_mode() {
        let mut probe = RegisterProbe::default();
        write_address::<(), _>(&mut probe, 0x30, 0x0123_4567_89ab_cdef, true).unwrap();
        assert_eq!(probe.last_offset, 0x34);
        assert_eq!(probe.last_value, 0x0123_4567);
    ...
    fn bounded_run_stop_receipt_requires_observed_start_and_halt() {
        ...
        assert_ne!(probe.command & USBCMD_INTERRUPT_ENABLE, 0);
