...
//! physical pages come independently from Arach's global frame ledger,
...
/// Production scatter-page capacity for one Hermes DMA service (96 MiB at
/// 4 KiB).
///
/// NVIDIA's supported Turing-and-later GSP-RM images include an 84 MiB GA10x
/// image. The ledger is deliberately sized above that published class, rather
/// than accepting a giant image through an unbounded allocator. A production
/// platform must place this fixed ledger in static kernel storage, never on a
/// boot or interrupt stack.
pub const MAXIMUM_PRODUCTION_HERMES_DMA_PAGES: usize = 24_576;

/// Test binaries deliberately use a smaller fixed ledger so each independent
/// unit test can keep its isolated fixture on a regular host thread stack.
/// Native Arach builds always select the production bound above.
#[cfg(test)]
pub const MAXIMUM_HERMES_DMA_PAGES: usize = 512;
#[cfg(not(test))]
pub const MAXIMUM_HERMES_DMA_PAGES: usize = MAXIMUM_PRODUCTION_HERMES_DMA_PAGES;
...
impl<'a, 'storage> FrameBackedHermesDma<'a, 'storage> {
    ...
    fn page_slot(
        ...
        let record = records.get(owner_slot)?;
        if record.phase == AllocationPhase::Empty
            || record.generation != owner_generation
            || page_index >= record.resident_pages
        {
            return None;
        }
        let owner = u16::try_from(owner_slot + 1).ok()?;
        let index = u16::try_from(page_index).ok()?;
        let slot = record.page_start.checked_add(page_index)?;
        let page = pages.get(slot)?;
        (page.phase != PagePhase::Empty
            && page.owner_slot == owner
            && page.owner_generation == owner_generation
            && page.page_index == index)
            .then_some(slot)
    }

    fn first_free_page_run(
        pages: &[PageRecord; MAXIMUM_HERMES_DMA_PAGES],
        count: usize,
    ) -> Option<usize> {
        if count == 0 || count > pages.len() {
            return None;
        }
        let mut run_start = 0_usize;
        let mut run_length = 0_usize;
        for (index, page) in pages.iter().enumerate() {
            if page.phase == PagePhase::Empty {
                if run_length == 0 {
                    run_start = index;
                }
                run_length += 1;
    ...
    fn reclaim_record(&self, state: &mut DmaState, slot: usize) -> Status {
        ...
        for page_index in (0..record.resident_pages).rev() {
            let Some(page_slot) = Self::page_slot(
                &state.records,
                &state.pages,
                slot,
                record.generation,
...
impl HermesDomainDmaService for FrameBackedHermesDma<'_, '_> {
    ...
    fn allocate(
        ...
        let slot = state
            ...
            .ok_or(STATUS_BUSY)?;
        let page_start = Self::first_free_page_run(&state.pages, frame_count).ok_or(STATUS_BUSY)?;
        let iova = state
            .iovas
        ...
        for page_index in 0..frame_count {
            let page_slot = page_start + page_index;
            if state.pages[page_slot].phase != PagePhase::Empty {
                state.poisoned = true;
        ...
        for page_index in 0..frame_count {
            let page_slot =
                Self::page_slot(&state.records, &state.pages, slot, generation, page_index)
