// kernel/arach/src/drivernet_host.rs
//! Arach integration host for measured GPU discovery and fail-closed activation.
...
pub struct ArachDriverClock;

impl DriverNetClock for ArachDriverClock {
    fn now_tick(&self) -> u64 {
        <crate::arch::Active as crate::arch::Architecture>::counter_sample()
    }
}

pub struct ArachIsolationBroker;

impl DeviceIsolationBroker for ArachIsolationBroker {
    fn begin(
        &self,
...
pub struct ArachNativeHost {
    portability_secret: u64,
}

impl ArachNativeHost {
...
impl NativeStrategyHost for ArachNativeHost {
    fn begin(
        &self,
        strategy: DriverStrategy,
        fingerprint: &GpuFingerprint,
...
pub struct ArachFirmwareHost {
    secret: u64,
}

impl ArachFirmwareHost {
    pub const fn new(secret: u64) -> Self {
        Self { secret }
    }
}

impl FirmwareFramebufferHost for ArachFirmwareHost {
    fn now_tick(&self) -> u64 {
        <crate::arch::Active as crate::arch::Architecture>::counter_sample()
    }
...
>(
    ...
    let driver_clock = ArachDriverClock;
    let device_isolation_broker = ArachIsolationBroker;

    let native_strategy_host = ArachNativeHost::new(secrets.dispatch ^ 0x4750_5550_4f52_5401);
    let firmware_framebuffer_host =
        ArachFirmwareHost::new(secrets.dispatch ^ 0x4657_4449_5350_4c01);
...
mod tests {
    ...
    fn hermes_handoff_rejects_incomplete_discovery_before_missing_iommu_backend() {
        ...
        let broker = ArachIsolationBroker;
