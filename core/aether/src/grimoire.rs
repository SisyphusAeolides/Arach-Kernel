...
/// Bounded pull of one trusted pointing-device packet.  The kernel owns the
/// controller and only the exact live Crest image may receive the normalized
/// motion; no raw I/O port or IRQ capability crosses this ABI.
pub const SYS_INPUT_NEXT: usize = 100;

/// Read-only measured-service status.  The kernel authenticates the caller
/// against the live image registry before exposing any status word.
pub const SYS_AEGIS_STATUS: usize = 101;

/// Bounded pull of one trusted normalized keyboard event. Boulder translates
/// the controller stream to stable Set-1-style key codes and authenticates
/// the exact live Crest image before copying one event to user memory.
pub const SYS_INPUT_KEY_NEXT: usize = 103;
